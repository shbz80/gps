"""
Plotter 2D - Modified from the plotter_3d.py file
...
class Plotter2D:
    def __init__(self, fig, gs, num_plots, rows=None, cols=None):
        ...
        self._axarr = [plt.subplot(self._gs_plots[i]) for i in range(num_plots)]
    ...
    def plot(self, i, xs, ys, linestyle='-', linewidth=1.0, marker=None,
            markersize=5.0, markeredgewidth=1.0, color='black', alpha=1.0, label=''):
        # Manually clip at xlim, ylim
        if self._lims[i]:
            xlim, ylim = self._lims[i]
    ...
    def plot_2d_points(self, i, points, linestyle='-', linewidth=1.0,
            marker=None, markersize=5.0, markeredgewidth=1.0, color='black',
            alpha=1.0, label=''):
        self.plot(i, points[:, 0], points[:, 1],
